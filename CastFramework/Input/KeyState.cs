namespace CastFramework
{
    internal struct KeyState
    {
        ...
        public KeyState(Key[] keys) : this()
        {
            ...
            foreach (var key in keys) SetKey(key);
        }

        public bool this[Key key] => GetKey(key);

        public uint PressedCount => CountBits(keys0) + CountBits(keys1) + CountBits(keys2) + CountBits(keys3)
        ...
        public Key[] GetPressedKeys()
        {
            ...
            if (keys7 != 0) AddKeysToArray(keys7, 7 * 32, keys, index);
